...
def run_small_shallownet(train_sample, test_sample):
    main_compute([shallow_args], [dl_dataset_args],
                 train_sample, test_sample,
                 sample_size_list, saving_params)


def run_small_handcrafted_features(train_sample, test_sample):
    main_compute([hf_args], [hf_dataset_args],
                 train_sample, test_sample,
                 sample_size_list, saving_params)


def run_small_full(train_sample, test_sample):
...
if __name__ == "__main__":
    train_sample, test_sample = get_epochs_data(
        ...
        test_subjects=[2],
        recording=[1])
    run_small_full(train_sample, test_sample)
